...
// Raw HTTP response body from POST /checkout/v1/payment — wrapped in "response".
export interface DokuPaymentApiResponse {
  message: string[];
  response: {
    order: {
    ...
    payment: {
      token_id:     string; // generated by DOKU — used as our paymentInvoiceToken
      url:          string; // payment page URL — redirect user here
      expired_date: string; // format yyyyMMddHHmmss, UTC+7
    };
  };
}

// Unwrapped shape returned by createDokuPayment() for callers.
